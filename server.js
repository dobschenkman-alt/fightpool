...
try{const{name,date,lockTime}=req.body;await db.events.update({_id:req.params.id},{$set:{name,date,lockTime:lockTime||null}});res.json({ok:true});}catch(e){res.status(500).json({error:e.message});}
});
